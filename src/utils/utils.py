...
def get_colors(num=1):
    colors = np.concatenate((np.random.randint(low=0, high=255,size=[num, 1], dtype=np.uint8),
                                np.random.randint(low=150, high=255,size=[num, 1], dtype=np.uint8),
                                np.random.randint(low=200, high=255,size=[num, 1], dtype=np.uint8)), axis=1)
    colors = np.array([colors])
    colors = cv2.cvtColor(colors, cv2.COLOR_HSV2RGB)[0]
    colors = colors 
